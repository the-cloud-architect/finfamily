...
export default function FourCast() {
  ...
  const saveScenario = () => { if (!scenarioName.trim()) return; setScenarios(prev => [...prev.filter(s => s.name !== scenarioName), { name: scenarioName, data: JSON.parse(JSON.stringify(data)), age, spouseAge, retireAge, depAges }]); setActiveScenario(scenarioName); };
  const saveCurrentScenario = () => { if (!activeScenario) return; setScenarios(prev => prev.map(s => s.name === activeScenario ? { ...s, data: JSON.parse(JSON.stringify(data)), age, spouseAge, retireAge, depAges } : s)); };
  const loadScenario = s => { setData(s.data); setAge(s.age); setSpouseAge(s.spouseAge || 43); setRetireAge(s.retireAge); setDepAges(s.depAges || [10,8,5]); setActiveScenario(s.name); setScenarioName(s.name); };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)", fontFamily: "'SF Mono', monospace", color: "#e2e8f0", padding: "12px" }}>
      <header style={{ textAlign: "center", marginBottom: "16px" }}><AnimatedTitle /><p style={{ color: "#64748b", fontSize: "11px", margin: "4px 0 0", letterSpacing: "2px" }}>35-YEAR HOUSEHOLD PROJECTION</p></header>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", marginBottom: "12px", justifyContent: "center", alignItems: "center" }}>
        ...
        {activeScenario && <button onClick={saveCurrentScenario} style={{ padding: "6px 12px", background: "linear-gradient(135deg, #3b82f6, #2563eb)", border: "none", borderRadius: "4px", color: "#fff", fontSize: "9px", cursor: "pointer", fontWeight: "bold" }}>💾 Save "{activeScenario}"</button>}
        <div style={{ display: "flex", alignItems: "center", gap: "6px", background: "rgba(30,41,59,0.8)", padding: "6px 10px", borderRadius: "6px" }}>
          ...
          <button onClick={saveScenario} style={{ padding: "3px 8px", background: "#22c55e", border: "none", borderRadius: "4px", color: "#fff", fontSize: "9px", cursor: "pointer" }}>Save As</button>
        </div>
        {scenarios.map(s => (
          <div key={s.name} style={{ display: "flex", alignItems: "center", gap: "4px", background: activeScenario === s.name ? "rgba(34,197,94,0.3)" : "rgba(99,102,241,0.2)", padding: "3px 8px", borderRadius: "4px", border: activeScenario === s.name ? "1px solid #22c55e" : "none" }}>
            <button onClick={() => loadScenario(s)} style={{ background: "none", border: "none", color: activeScenario === s.name ? "#4ade80" : "#a78bfa", fontSize: "9px", cursor: "pointer" }}>{s.name}</button>
            <button onClick={() => { setScenarios(p => p.filter(x => x.name !== s.name)); if (activeScenario === s.name) setActiveScenario(null); }} style={{ background: "none", border: "none", color: "#f87171", fontSize: "9px", cursor: "pointer" }}>×</button>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: "rgba(30,41,59,0.8)", borderRadius: "8px", padding: "12px", border: "1px solid rgba(148,163,184,0.2)", marginBottom: "12px", fontSize: "9px", color: "#94a3b8" }}>
        <strong style={{ color: "#a78bfa" }}>📋 Tips:</strong> <span style={{ color: "#4ade80" }}>Green</span>=Year 1 inputs | <span style={{ color: "#60a5fa" }}>Blue</span>=calculated | <span style={{ color: "#fb923c" }}>Orange</span>=retirement | Click headers to expand/collapse
      </div>
