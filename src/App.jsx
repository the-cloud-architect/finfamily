...
const Cell = ({ value, onChange, pct, num, isYear0, isIncome, isExpense }) => {
  ...
  const textColor = isYear0 ? "#000" : isIncome ? "#4ade80" : isExpense ? "#f87171" : "#fff";
  ...
  return <span onClick={() => { setTmp(value); setEd(true); }} style={{ cursor: "pointer", padding: "2px 4px", borderRadius: "3px", fontSize: "10px", background: isYear0 ? "#fbbf24" : "rgba(59,130,246,0.1)", border: isYear0 ? "2px solid #f59e0b" : "1px dashed rgba(59,130,246,0.3)", display: "inline-block", minWidth: "45px", textAlign: "center", color: textColor }}>{num ? value : pct ? value+"%" : fmt(value)}</span>;
};
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f172a 0%, #1e293b 100%)", fontFamily: "'SF Mono', monospace", color: "#e2e8f0", padding: "12px" }}>
      ...
      <div style={{ display: "flex", gap: "6px", marginBottom: "12px", justifyContent: "center" }}>
        {[["variables", "⚙️ Forecast"], ["summary", "📊 Summary"], ["charts", "📈 Charts"]].map(([k, v]) => <button key={k} onClick={() => setTab(k)} style={{ padding: "6px 16px", background: tab === k ? "linear-gradient(135deg, #6366f1, #4f46e5)" : "transparent", border: tab === k ? "none" : "1px solid rgba(148,163,184,0.2)", borderRadius: "5px", color: tab === k ? "#fff" : "#94a3b8", fontSize: "11px", fontWeight: "600", cursor: "pointer" }}>{v}</button>)}
      </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "9px" }}>
              ...
              <tbody>
                <AccordionSection title="INCOME" icon="💰" color="#22c55e" isOpen={sections.income} onToggle={() => toggleSection('income')}>
                  {[["cashIncome", "Cash"], ["rsuIncome", "RSU"], ["match401k", "401k Match"], ["investmentIncome", "Investment Income"], ["rentalIncome", "Rental Income"]].map(([k, l]) => <tr key={k}><td style={{...stickyTd, color: "#4ade80"}}>{l}</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}><Cell value={data[i][k]} onChange={v => update(i, k, v)} isYear0={i === 0} isIncome={true} /></td>)}</tr>)}
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Tax Rate %</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}><Cell value={data[i].taxRate} onChange={v => update(i, "taxRate", v)} pct isYear0={i === 0} isExpense={true} /></td>)}</tr>
...
<AccordionSection title="LIABILITIES" icon="📉" color="#f87171" isOpen={sections.liabilities} onToggle={() => toggleSection('liabilities')}>
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Primary Mtg Bal</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[0].mortgageBalance} onChange={v => updateMortgage("mortgageBalance", v)} isYear0={true} isExpense={true} /> : <span style={{color: r.mortgageBalEnd > 0 ? "#f87171" : "#4ade80"}}>{fmt(r.mortgageBalEnd)}</span>}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#94a3b8"}}>↳ Rate%/Yrs</td><td style={{ ...cellStyle, background: bg(enriched[0].year, 0) }}><Cell value={data[0].mortgageRate} onChange={v => updateMortgage("mortgageRate", v)} pct isYear0={true} isExpense={true} /> <Cell value={data[0].mortgageYears} onChange={v => updateMortgage("mortgageYears", v)} num isYear0={true} /></td>{enriched.slice(1).map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i+1) }}></td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Rental Mtg Bal</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[0].rentalMortgageBalance} onChange={v => updateRentalMortgage("rentalMortgageBalance", v)} isYear0={true} isExpense={true} /> : <span style={{color: r.rentalMortgageBalEnd > 0 ? "#f87171" : "#4ade80"}}>{fmt(r.rentalMortgageBalEnd)}</span>}</td>)}</tr>
                  ...
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Car Loan Bal</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[0].carLoanBalance} onChange={v => updateCarLoan("carLoanBalance", v)} isYear0={true} isExpense={true} /> : <span style={{color: r.carLoanBalEnd > 0 ? "#f87171" : "#4ade80"}}>{fmt(r.carLoanBalEnd)}</span>}</td>)}</tr>
                  ...
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Other Debt</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}><Cell value={data[i].otherDebt} onChange={v => updateSingleYear(i, "otherDebt", v)} isYear0={i === 0} isExpense={true} /></td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#f87171"}}>HELOC Used</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}><Cell value={data[i].helocUsed} onChange={v => updateHelocUsed(i, v)} isYear0={i === 0} isExpense={true} /></td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#94a3b8"}}>↳ Rate %</td><td style={{ ...cellStyle, background: bg(enriched[0].year, 0) }}><Cell value={data[0].helocRate} onChange={v => updateYear0("helocRate", v)} pct isYear0={true} isExpense={true} /></td>{enriched.slice(1).map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i+1) }}></td>)}</tr>
                  <tr><td style={{ ...stickyTd, fontWeight: "bold", color: "#f87171" }}>Total Liabilities</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, fontWeight: "bold", color: "#f87171", background: bg(r.year, i) }}>{fmt(r.debt)}</td>)}</tr>
                </AccordionSection>

                <AccordionSection title="HOUSING COSTS" icon="🏠" color="#f97316" isOpen={sections.housing} onToggle={() => toggleSection('housing')}>
                  <tr><td style={{ ...stickyTd, color: "#f87171" }}>Primary Mtg Pmt</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, color: "#f87171", background: bg(r.year, i) }}>{fmt(r.effectiveMtgPayment)}</td>)}</tr>
                  <tr><td style={{ ...stickyTd, color: "#94a3b8" }}>↳ Interest</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, color: "#f87171", background: bg(r.year, i) }}>{fmt(r.mortgageInterest)}</td>)}</tr>
                  <tr><td style={{ ...stickyTd, color: "#94a3b8" }}>↳ Principal</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, color: "#4ade80", background: bg(r.year, i) }}>{fmt(r.mortgagePrincipal)}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Home Taxes</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[0].homeTaxes} onChange={v => updateYear0("homeTaxes", v)} isYear0={true} isExpense={true} /> : <span style={{color:"#f87171"}}>{fmt(r.homeTaxes)}</span>}</td>)}</tr>
                  <tr><td style={{ ...stickyTd, color: "#f87171" }}>Rental Mtg Pmt</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, color: "#f87171", background: bg(r.year, i) }}>{fmt(r.effectiveRentalPayment)}</td>)}</tr>
                  <tr><td style={{ ...stickyTd, color: "#94a3b8" }}>↳ Interest</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, color: "#f87171", background: bg(r.year, i) }}>{fmt(r.rentalInterest)}</td>)}</tr>
                  <tr><td style={{ ...stickyTd, color: "#94a3b8" }}>↳ Principal</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, color: "#4ade80", background: bg(r.year, i) }}>{fmt(r.rentalPrincipal)}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Rental Taxes</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[0].rentalTaxes} onChange={v => updateYear0("rentalTaxes", v)} isYear0={true} isExpense={true} /> : <span style={{color:"#f87171"}}>{fmt(r.rentalTaxes)}</span>}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#f87171"}}>HOA</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[0].hoaFees} onChange={v => updateYear0("hoaFees", v)} isYear0={true} isExpense={true} /> : <span style={{color:"#f87171"}}>{fmt(data[0].hoaFees)}</span>}</td>)}</tr>
                  ...
                  <tr><td style={{ ...stickyTd, fontWeight: "bold", color: "#f87171" }}>Total Housing</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, fontWeight: "bold", color: "#f87171", background: bg(r.year, i) }}>{fmt(r.housingCost)}</td>)}</tr>
                </AccordionSection>
                ...
                <AccordionSection title={`OTHER EXPENSES (${data[0].expenseInflationRate}% inflation)`} icon="💸" color="#ef4444" isOpen={sections.expenses} onToggle={() => toggleSection('expenses')}>
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Inflation %</td><td style={{ ...cellStyle, background: bg(enriched[0].year, 0) }}><Cell value={data[0].expenseInflationRate} onChange={v => updateYear0("expenseInflationRate", v)} pct isYear0={true} isExpense={true} /></td>{enriched.slice(1).map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i+1) }}></td>)}</tr>
                  {[["utilitiesExpense", "Utilities"], ["foodExpense", "Food"], ["clothingExpense", "Clothing"], ["entertainmentExpense", "Entertainment"], ["vacationBudget", "Vacation"], ["educationExpense", "Education"], ["healthcareExpense", "Healthcare"], ["healthInsurance", "Health Ins"], ["carInsurance", "Car Ins"], ["homeInsurance", "Home Ins"], ["transportExpense", "Transport"], ["miscExpense", "Misc"]].map(([k, l]) => (
                    <tr key={k}><td style={{...stickyTd, color: "#f87171"}}>{l}</td><td style={{ ...cellStyle, background: bg(enriched[0].year, 0) }}>{k === 'vacationBudget' ? <Cell value={data[0][k]} onChange={v => update(0, k, v)} isYear0={true} isExpense={true} /> : <Cell value={data[0][k]} onChange={v => updateYear0(k, v)} isYear0={true} isExpense={true} />}</td>{enriched.slice(1).map((r, i) => <td key={r.year} style={{ ...cellStyle, color: "#f87171", background: bg(r.year, i+1) }}>{k === 'vacationBudget' ? fmt(r.vacationBudget) : fmt(data[0][k] * Math.pow(1 + data[0].expenseInflationRate / 100, i+1))}</td>)}</tr>
                  ))}
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Car Maint %</td><td style={{ ...cellStyle, background: bg(enriched[0].year, 0) }}><Cell value={data[0].carMaintenanceRate} onChange={v => updateYear0("carMaintenanceRate", v)} pct isYear0={true} isExpense={true} /></td>{enriched.slice(1).map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i+1) }}></td>)}</tr>
                  <tr><td style={{ ...stickyTd, color: "#f87171" }}>Car Maintenance</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, color: "#f87171", background: bg(r.year, i) }}>{fmt(r.carMaint)}</td>)}</tr>
                  <tr><td style={{ ...stickyTd, color: "#f87171" }}>Car Loan Pmt</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, color: r.carLoanPaymentEffective > 0 ? "#f87171" : "#64748b", background: bg(r.year, i) }}>{fmt(r.carLoanPaymentEffective)}</td>)}</tr>
                ...
                <AccordionSection title="ASSETS" icon="🏦" color="#4ade80" isOpen={sections.assets} onToggle={() => toggleSection('assets')}>
                  <tr><td style={{...stickyTd, color: "#4ade80"}}>Cash</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}><Cell value={data[i].cashBalance} onChange={v => updateSingleYear(i, "cashBalance", v)} isYear0={i === 0} isIncome={true} /></td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#4ade80"}}>Stocks/Bonds</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[i].stocksBonds} onChange={v => updateYear0("stocksBonds", v)} isYear0={true} isIncome={true} /> : <span style={{color:"#4ade80"}}>{fmt(r.stocks)}</span>}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#94a3b8"}}>↳ Growth %</td><td style={{ ...cellStyle, background: bg(enriched[0].year, 0) }}><Cell value={data[0].stockGrowthRate} onChange={v => updateYear0("stockGrowthRate", v)} pct isYear0={true} isIncome={true} /></td>{enriched.slice(1).map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i+1) }}></td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#4ade80"}}>401k</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[i].retirement401k} onChange={v => updateYear0("retirement401k", v)} isYear0={true} isIncome={true} /> : <span style={{color:"#4ade80"}}>{fmt(r.ret401k)}</span>}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#94a3b8"}}>↳ Growth %</td><td style={{ ...cellStyle, background: bg(enriched[0].year, 0) }}><Cell value={data[0].retirement401kGrowthRate} onChange={v => updateYear0("retirement401kGrowthRate", v)} pct isYear0={true} isIncome={true} /></td>{enriched.slice(1).map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i+1) }}></td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#4ade80"}}>Home</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[i].primaryHomeValue} onChange={v => updateYear0("primaryHomeValue", v)} isYear0={true} isIncome={true} /> : <span style={{color:"#4ade80"}}>{fmt(r.home)}</span>}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#94a3b8"}}>↳ Growth %</td><td style={{ ...cellStyle, background: bg(enriched[0].year, 0) }}><Cell value={data[0].homeGrowthRate} onChange={v => updateYear0("homeGrowthRate", v)} pct isYear0={true} isIncome={true} /></td>{enriched.slice(1).map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i+1) }}></td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#4ade80"}}>Rental</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[i].rentalPropertyValue} onChange={v => updateYear0("rentalPropertyValue", v)} isYear0={true} isIncome={true} /> : <span style={{color:"#4ade80"}}>{fmt(r.rental)}</span>}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#4ade80"}}>Car</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[i].carValue} onChange={v => updateYear0("carValue", v)} isYear0={true} isIncome={true} /> : <span style={{color:"#f87171"}}>{fmt(r.car)}</span>}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#94a3b8"}}>↳ Deprec %</td><td style={{ ...cellStyle, background: bg(enriched[0].year, 0) }}><Cell value={data[0].carDepreciation} onChange={v => updateYear0("carDepreciation", v)} pct isYear0={true} isExpense={true} /></td>{enriched.slice(1).map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i+1) }}></td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#4ade80"}}>Machines</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[i].otherMachines} onChange={v => updateYear0("otherMachines", v)} isYear0={true} isIncome={true} /> : <span style={{color:"#f87171"}}>{fmt(r.mach)}</span>}</td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#4ade80"}}>HELOC Limit</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}>{i === 0 ? <Cell value={data[i].helocLimit} onChange={v => updateSingleYear(i, "helocLimit", v)} isYear0={true} isIncome={true} /> : <span style={{color:"#4ade80"}}>{fmt(data[0].helocLimit)}</span>}</td>)}</tr>
                  <tr><td style={{ ...stickyTd, fontWeight: "bold", color: "#4ade80" }}>Total Assets</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, fontWeight: "bold", color: "#4ade80", background: bg(r.year, i) }}>{fmt(r.assets)}</td>)}</tr>
                </AccordionSection>

                

                <AccordionSection title="MAJOR PURCHASES" icon="🛒" color="#8b5cf6" isOpen={sections.purchases} onToggle={() => toggleSection('purchases')}>
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Major Purchase</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}><Cell value={data[i].majorPurchase} onChange={v => updateSingleYear(i, "majorPurchase", v)} isYear0={i === 0} isExpense={true} /></td>)}</tr>
                  <tr><td style={{...stickyTd, color: "#f87171"}}>Car Purchase</td>{enriched.map((r, i) => <td key={r.year} style={{ ...cellStyle, background: bg(r.year, i) }}><Cell value={data[i].carPurchase} onChange={v => updateSingleYear(i, "carPurchase", v)} isYear0={i === 0} isExpense={true} /></td>)}</tr>
                </AccordionSection>
      ...
      {tab === "charts" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <div style={{ background: "rgba(30,41,59,0.6)", borderRadius: "10px", padding: "12px", border: "1px solid rgba(148,163,184,0.1)" }}>
            ...
            <div style={{ fontSize: "9px", color: "#64748b", marginBottom: "6px" }}><span style={{ color: "#a78bfa" }}>■</span> Net Worth (left) | <span style={{ color: "#f97316" }}>■</span> Mortgages (right)</div>
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={enriched.map(r => ({ year: r.year, nw: r.netWorth, primaryMtg: r.mortgageBalEnd, rentalMtg: r.rentalMortgageBalEnd }))}>
                ...
                <YAxis yAxisId="left" stroke="#a78bfa" fontSize={9} tickFormatter={fmt} orientation="left"/>
                <YAxis yAxisId="right" stroke="#f97316" fontSize={9} tickFormatter={fmt} orientation="right"/>
                <Tooltip contentStyle={{ background: "#1e293b", border: "1px solid #475569", borderRadius: "4px", fontSize: "10px" }} formatter={fmt}/>
                <Legend wrapperStyle={{ fontSize: "10px" }}/>
                <ReferenceLine x={retireYear} stroke="#fb923c" strokeDasharray="5 5" label={{ value: `Retire`, fill: '#fb923c', fontSize: 9 }} />
